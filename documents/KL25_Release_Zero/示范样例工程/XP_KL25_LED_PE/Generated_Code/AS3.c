...
byte AS3_RecvChar(AS3_TComData *Chr)
{
  ...
  if (AS3_InpLen > 0x00U) {            /* Is number of received chars greater than 0? */
    EnterCritical();                   /* Disable global interrupts */
    AS3_InpLen--;                      /* Decrease number of received chars */
    *Chr = InpBuffer[InpIndexR++];     /* Received char */
    if (InpIndexR >= AS3_INP_BUF_SIZE) { /* Is the index out of the receive buffer? */
...
{
  if (AS3_OutLen == AS3_OUT_BUF_SIZE) { /* Is number of chars in buffer is the same as a size of the transmit buffer */
    return ERR_TXFULL;                 /* If yes then error */
  }
  EnterCritical();                     /* Disable global interrupts */
  AS3_OutLen++;                        /* Increase number of bytes in the transmit buffer */
  OutBuffer[OutIndexW++] = Chr;        /* Store char to buffer */
  if (OutIndexW >= AS3_OUT_BUF_SIZE) { /* Is the pointer out of the transmit buffer */
  ...
  if ((SerFlag & RUNINT_FROM_TX) == 0U) {
    ...
    (void)ASerialLdd3_SendBlock(ASerialLdd3_DeviceDataPtr, (LDD_TData *)&OutBuffer[OutIndexR], 1U); /* Send one data byte */
  }
  ExitCritical();                      /* Enable global interrupts */
  return ERR_OK;                       /* OK */
}
...
byte AS3_RecvBlock(AS3_TComData *Ptr, word Size, word *Rcv)
{
  ...
  for (count = 0x00U; count < Size; count++) {
    switch (AS3_RecvChar(Ptr++)) {     /* Receive data and test the return value*/
...
byte AS3_SendBlock(AS3_TComData *Ptr, word Size, word *Snd)
{
  ...
  AS3_TComData *TmpPtr = Ptr;          /* Temporary output buffer pointer */
  bool tmpOnFreeTxBufSemaphore = OnFreeTxBufSemaphore; /* Local copy of OnFreeTxBufSemaphore state */

  while ((count < Size) && (AS3_OutLen < AS3_OUT_BUF_SIZE)) { /* While there is some char desired to send left and output buffer is not full do */
    ...
    if (OutIndexW >= AS3_OUT_BUF_SIZE) { /* Is the index out of the transmit buffer? */
      OutIndexW = 0x00U;               /* Set index to the first item in the transmit buffer */
    }
    count++;                           /* Increase the count of sent data */
    if ((count == Size) || (AS3_OutLen == AS3_OUT_BUF_SIZE)) { /* Is the last desired char put into buffer or the buffer is full? */
...
{
  EnterCritical();                     /* Disable global interrupts */
  AS3_InpLen = 0x00U;                  /* Set number of chars in the transmit buffer to 0 */
...
{
  EnterCritical();                     /* Disable global interrupts */
  AS3_OutLen = 0x00U;                  /* Set number of chars in the receive buffer to 0 */
...
{
  return AS3_InpLen;                   /* Return number of chars in receive buffer */
}
...
{
  return AS3_OutLen;                   /* Return number of chars in the transmitter buffer */
}
...
void AS3_Init(void)
{
  ...
  AS3_InpLen = 0x00U;                  /* No char in the receive buffer */
  InpIndexR = 0x00U;                   /* Set index on the first item in the receive buffer */
  InpIndexW = 0x00U;
  AS3_OutLen = 0x00U;                  /* No char in the transmit buffer */
...
void ASerialLdd3_OnBlockReceived(LDD_TUserData *UserDataPtr)
{
  ...
  if (AS3_InpLen < AS3_INP_BUF_SIZE) { /* Is number of bytes in the receive buffer lower than size of buffer? */
    AS3_InpLen++;                      /* Increase number of chars in the receive buffer */
    InpBuffer[InpIndexW++] = (AS3_TComData)BufferRead; /* Save received char to the receive buffer */
    if (InpIndexW >= AS3_INP_BUF_SIZE) { /* Is the index out of the receive buffer? */
      InpIndexW = 0x00U;               /* Set index on the first item into the receive buffer */
    }
    Flags |= ON_RX_CHAR;               /* If yes then set the OnRxChar flag */
    if (AS3_InpLen == AS3_INP_BUF_SIZE) { /* Is number of bytes in the receive buffer equal to the size of buffer? */
  ...
  if ((Flags & ON_ERROR) != 0U) {      /* Is any error flag set? */
    AS3_OnError();                     /* Invoke user event */
  } else {
    if ((Flags & ON_RX_CHAR) != 0U) {  /* Is OnRxChar flag set? */
      AS3_OnRxChar();                  /* Invoke user event */
    }
    if ((Flags & ON_FULL_RX) != 0U) {  /* Is OnTxChar flag set? */
      AS3_OnFullRxBuf();               /* Invoke user event */
    }
  }
  (void)ASerialLdd3_ReceiveBlock(ASerialLdd3_DeviceDataPtr, &BufferRead, 1U); /* Receive one data byte */
}
...
void ASerialLdd3_OnBlockSent(LDD_TUserData *UserDataPtr)
{
  ...
  OutIndexR++;
  if (OutIndexR >= AS3_OUT_BUF_SIZE) { /* Is the index out of the transmit buffer? */
    OutIndexR = 0x00U;                 /* Set index on the first item into the transmit buffer */
  }
  AS3_OutLen--;                        /* Decrease number of chars in the transmit buffer */
  if (AS3_OutLen != 0U) {              /* Is number of bytes in the transmit buffer greater then 0? */
    SerFlag |= RUNINT_FROM_TX;         /* Set flag "running int from TX"? */
    (void)ASerialLdd3_SendBlock(ASerialLdd3_DeviceDataPtr, (LDD_TData *)&OutBuffer[OutIndexR], 1U); /* Send one data byte */
  } else {
  ...
  if ((OnFlags & ON_FREE_TX) != 0x00U) { /* Is flag "OnFreeTxBuf" set? */
    AS3_OnFreeTxBuf();                 /* If yes then invoke user event */
  }
}
...
void ASerialLdd3_OnError(LDD_TUserData *UserDataPtr)
{
  ...
  (void)UserDataPtr;                   /* Parameter is not used, suppress unused argument warning */
  (void)ASerialLdd3_GetError(ASerialLdd3_DeviceDataPtr, &SerialErrorMask); /* Get error state */
